...
# 			print(order)
	...
	params = {
			...
			'total':subtotal+tax+delev,
			'category':category.objects.all(),
			'p':(subtotal+tax+delev)*100
		}
	return render(request, 'main/checkout.html', params)
from HexaEdge.settings import RAZORPAY_API_KEY, RAZORPAY_API_SECRET_KEY
...
def xyz(request):
	if request.method=='POST':
		...
		context['order_id']= payment_order_id
		context['amount']=order_amount
	return render(request, 'main/checkout2.html',context)
def payment_status(request):
    ...
    # client instance
    client = razorpay.Client(auth=('rzp_test_Wdk6BGRM5zd4Jd', '1s5wWncnl9zl5IEAxSQVp6vp'))

    try:
        status = client.utility.verify_payment_signature(params_dict)
        return render(request, 'success.html', {'status': True})
    except:
        return render(request, 'sucess.html', {'status': False})
